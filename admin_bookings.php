...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="admin_bookings.php" class="sidebar-link active"><i class="fas fa-ticket-alt"></i> Bookings</a></li>
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-4 col-sm-12">
                        <div class="input-group">
                            ...
                            <input type="text" name="search" class="form-control" placeholder="Search bookings..." value="<?= htmlspecialchars($search_term) ?>">
                        </div>
                    </div>
                            ...
                            <?php if ($result && $result->num_rows > 0): ?>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($row['passenger_name']) ?><br><small><?= htmlspecialchars($row['passenger_email']) ?></small></td>
                                        <td><?= htmlspecialchars($row['from_location']) ?> to <?= htmlspecialchars($row['to_location']) ?></td>
                                        <td><?= date('M d, Y', strtotime($row['journey_date'])) ?> <?= htmlspecialchars($row['boarding_time'] ?? '') ?></td>
                                        <td>₹<?= number_format($row['total_amount'], 2) ?></td>
                                        <td><span class="status-badge <?= $statusClass ?>"><?= htmlspecialchars($row['booking_status']) ?></span></td>
                                        <td>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="ticket-detail-row"><span class="ticket-label">Booking ID:</span><span id="modal-ticket-id"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Ticket ID:</span><span id="modal-ticket-id-specific"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Passenger:</span><span id="modal-passenger"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Email:</span><span id="modal-email"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">From:</span><span id="modal-from"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">To:</span><span id="modal-to"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Journey Date:</span><span id="modal-journey-date"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Departure:</span><span id="modal-departure-time"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Total Amount:</span><span id="modal-amount"></span></div>
                    <div class="ticket-detail-row"><span class="ticket-label">Status:</span><span class="status-badge" id="modal-status"></span></div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('click', (e) => {
            if (window.innerWidth < 992 && !sidebar.contains(e.target) && !menuToggle.contains(e.target) && sidebar.classList.contains('show')) {
        ...
        document.querySelectorAll('.view-ticket').forEach(button => {
            button.addEventListener('click', function () {
                ...
                document.getElementById('modal-amount').textContent = '₹' + getValue(Number(data.totalAmount).toFixed(2));
