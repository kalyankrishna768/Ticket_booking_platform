...
<body class="bg-gray-50">
    <div id="sidebarBackdrop" class="fixed inset-0 bg-black bg-opacity-50 z-30 sidebar-backdrop md:hidden" onclick="toggleSidebar()"></div>
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto main-content">
            ...
            <header class="bg-white shadow-sm hidden md:block">
                <div class="flex items-center justify-between px-4 md:px-6 py-4">
                    <h1 class="text-xl font-semibold"><i class="fas fa-clipboard-check mr-2"></i>New Bookings Management</h1>
            ...
            <?php if (isset($_GET['success']) && $_GET['success'] == 1): ?>
                <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mx-4 my-4" role="alert">
                    <p><i class="fas fa-check-circle mr-2"></i>Booking successfully <?php echo $_GET['action'] === 'confirm' ? 'confirmed' : 'cancelled'; ?>!</p>
                </div>
            <?php endif; ?>
            <?php if (isset($_GET['error']) && $_GET['error'] == 1): ?>
                <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mx-4 my-4" role="alert">
                    <p><i class="fas fa-exclamation-circle mr-2"></i>Failed to <?php echo $_GET['action']; ?> booking. Please try again.</p>
                </div>
            <?php endif; ?>
            ...
            <main class="p-4 md:p-6">
                    ...
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <form method="GET" action="" class="flex items-center w-full md:w-auto">
                            <div class="relative flex-grow">
                                ...
                                <input type="text" name="search" placeholder="Search bookings..." value="<?php echo htmlspecialchars($searchQuery); ?>" class="pl-10 pr-16 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full">
                                <button type="submit" class="absolute right-0 top-0 bottom-0 px-3 bg-blue-600 text-white rounded-r-lg hover:bg-blue-700">
                                    <i class="fas fa-search mr-1"></i>
                                </button>
                            </div>
                        </form>
                        <div class="filter-buttons flex gap-2">
                            <button onclick="window.location.href='x_bookings.php<?php echo $searchQuery ? "?search=" . urlencode($searchQuery) : ""; ?>'" class="filter-button px-4 py-2 border rounded-lg <?php echo $filterDate === 'all' ? 'active' : 'bg-white text-blue-600 hover:bg-blue-100'; ?>">All Bookings</button>
                            <button onclick="window.location.href='x_bookings.php?filter_date=today<?php echo $searchQuery ? "&search=" . urlencode($searchQuery) : ""; ?>'" class="filter-button px-4 py-2 border rounded-lg <?php echo $filterDate === 'today' ? 'active' : 'bg-white text-blue-600 hover:bg-blue-100'; ?>">Today's Bookings</button>
                            <button onclick="openDateRangeModal()" class="filter-button px-4 py-2 border rounded-lg <?php echo $filterDate === 'range' ? 'active' : 'bg-white text-blue-600 hover:bg-blue-100'; ?>">Select Date Range</button>
                        </div>
                ...
                <div id="selectDateModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
                    ...
                    <div class="relative flex items-center justify-center min-h-screen p-4">
                        <div class="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white modal-content">
                            <div class="flex justify-between items-center mb-4">
                                <h3 class="text-lg font-medium text-gray-900"><i class="fas fa-calendar-alt text-blue-600 mr-2"></i>Select Date Range</h3>
                                <button type="button" onclick="closeDateRangeModal()" class="text-gray-400 hover:text-gray-500"><i class="fas fa-times"></i></button>
                            </div>
                            <form id="dateRangeForm" method="GET" action="">
                                <input type="hidden" name="filter_date" value="range">
                                <?php if ($searchQuery): ?><input type="hidden" name="search" value="<?php echo htmlspecialchars($searchQuery); ?>"><?php endif; ?>
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Start Date</label>
                                        <input type="date" name="start_date" id="startDateInput" value="<?php echo $startDate ?? date('Y-m-d'); ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">End Date</label>
                                        <input type="date" name="end_date" id="endDateInput" value="<?php echo $endDate ?? date('Y-m-d'); ?>" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3">
                                    </div>
                                </div>
                                <div class="mt-4 flex justify-end space-x-3">
                                    <button type="button" onclick="closeDateRangeModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">Cancel</button>
                                    <button type="button" onclick="applyDateRange()" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Apply</button>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-hashtag mr-1"></i> ID</th>
                                        <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-chair mr-1"></i> Seat</th>
                                        <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-user mr-1"></i> Passenger</th>
                                        <th class="hidden md:table-cell px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-calendar-day mr-1"></i> Journey Date</th>
                                        <th class="hidden md:table-cell px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-map-pin mr-1"></i> Route ID</th>
                                        <th class="hidden md:table-cell px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-map-marked-alt mr-1"></i> Route</th>
                                        <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-rupee-sign mr-1"></i> Amount</th>
                                        <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-tag mr-1"></i> Status</th>
                                        <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"><i class="fas fa-cog mr-1"></i> Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <?php foreach ($newBookings as $booking): ?>
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">#<?php echo htmlspecialchars($booking['id']); ?></td>
                                            <td class="px-4 md:px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($booking['seat_no']); ?></td>
                                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($booking['passenger_name']); ?></div>
                                                <div class="text-sm text-gray-500 hidden md:block"><?php echo htmlspecialchars($booking['phone_number']); ?></div>
                                            </td>
                                            <td class="hidden md:table-cell px-4 md:px-6 py-4 whitespace-nowrap"><?php echo date('d M Y', strtotime($booking['journey_date'])); ?></td>
                                            <td class="hidden md:table-cell px-4 md:px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($booking['route_id']); ?></td>
                                            <td class="hidden md:table-cell px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <div><?php echo htmlspecialchars($booking['from_location'] . ' - ' . $booking['to_location']); ?></div>
                                                <div class="text-xs"><?php echo htmlspecialchars($booking['boarding_point']) . ' → ' . htmlspecialchars($booking['dropping_point']); ?></div>
                                            </td>
                                            <td class="px-4 md:px-6 py-4 whitespace-nowrap font-medium text-green-600"><?php echo number_format($booking['total_amount'], 2); ?></td>
                                            <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                                <?php
                                                $statusClasses = [
                                            ...
                                            <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm">
                                                <button onclick="viewBooking(<?php echo $booking['id']; ?>)" class="text-blue-600 hover:text-blue-900 mr-2" title="View Details"><i class="fas fa-eye"></i></button>
                                                <?php if ($booking['booking_status'] === 'PENDING'): ?>
                                                    <button onclick="confirmBooking(<?php echo $booking['id']; ?>)" class="text-green-600 hover:text-green-900 mr-2" title="Confirm Booking"><i class="fas fa-check"></i></button>
                                                    <button onclick="cancelBooking(<?php echo $booking['id']; ?>)" class="text-red-600 hover:text-red-900" title="Cancel Booking"><i class="fas fa-ban"></i></button>
                                                <?php endif; ?>
                                                <button onclick="showBookingDetails(<?php echo $booking['id']; ?>)" class="md:hidden text-gray-600 hover:text-gray-900 ml-2" title="More Details"><i class="fas fa-info-circle"></i></button>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                ...
                <div id="bookingDetailsModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
                    ...
                    <div class="relative flex items-center justify-center min-h-screen p-4">
                        <div class="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white modal-content">
                            <div class="flex justify-between items-center mb-4">
                                <h3 class="text-lg font-medium text-gray-900"><i class="fas fa-ticket-alt mr-2"></i>Booking Details</h3>
                            ...
                            <div class="mt-4 flex justify-end space-x-2">
                                ...
                                <button type="button" onclick="closeModal()" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"><i class="fas fa-times mr-1"></i> Close</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function applyDateRange() {
            ...
            window.location.href = `x_bookings.php?filter_date=range&start_date=${startDate}&end_date=${endDate}<?php echo $searchQuery ? "&search=" . urlencode($searchQuery) : ""; ?>`;
        }
        ...
        function confirmBooking(id) {
            if (confirm('Are you sure you want to confirm this booking?')) {
                ...
                form.innerHTML = `<input type="hidden" name="booking_id" value="${id}"><input type="hidden" name="action" value="confirm">`;
        ...
        function cancelBooking(id) {
            if (confirm('Are you sure you want to cancel this booking?')) {
                ...
                form.method = 'POST';
                form.action = 'x_bookings.php';
                form.innerHTML = `<input type="hidden" name="booking_id" value="${id}"><input type="hidden" name="action" value="cancel">`;
                document.body.appendChild(form);
                form.submit();
            }
        }
