...
try {
    ...
    // Fetch all transactions for the user based on user_id (assuming wallet_id in wallet_transactions is the same as user_id)
    $transactions_stmt = $conn->prepare("SELECT type, amount, transaction_date AS date FROM wallet_transactions WHERE wallet_id = ? ORDER BY transaction_date DESC");
    $transactions_stmt->bind_param("i", $user_id); // Use user_id directly as wallet_id
    ...
    <div class="sticky top-0 bg-white shadow-md z-10">
        <div class="container mx-auto px-4">
            <div class="flex justify-between items-center py-3">
                <a href="wallet.php" class="flex items-center text-indigo-700 hover:text-indigo-500 transition">
                    ...
                    <div class="overflow-x-auto rounded-xl shadow-sm border border-gray-100">
                        <?php if (empty($transactions)): ?>
                            <div class="p-8 text-center">
                                ...
                                <p class="text-gray-500">Your transaction history will appear here once you make transactions.</p>
                            </div>
                        <?php else: ?>
                            <table class="w-full">
                                <thead>
